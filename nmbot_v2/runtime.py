...
from .contracts import DialogFocus, ExecutableTurn, ExecutionResult, IntentGoal, OptionCard, SafeTurnContext, SearchResult, SemanticPlan, Stage, StateDelta, TurnAction, TurnPlan, TurnResult
...
class TurnProcessor:
    ...
    def _delta_for(self, action: TurnAction, plan: TurnPlan, execution: ExecutionResult, state: ConversationState) -> StateDelta:
        ...
        if action == TurnAction.ANSWER_OFF_TOPIC:
            return StateDelta(clear_fields=("pending_followup",))
        ...
        if action == TurnAction.ANSWER_SELECTED_OPTION:
            ...
            pending = interrupted_contact or (FINANCING_CONSENT_FOLLOWUP if selected_financing else _selected_live_fact_pending(plan, execution))
            ...
            return StateDelta(
                ...
                operator_offered=True if selected_enrichment_failure else False,
                contact_consent=False if selected_enrichment_failure else None,
        ...
        if action == TurnAction.CLARIFY_FINANCING:
            ...
            return StateDelta(params_update=params_update, active_topic="financing", pending_followup=FINANCING_CONSENT_FOLLOWUP)
        if action == TurnAction.ANSWER_FROM_CURRENT_OPTIONS:
            ...
            return StateDelta(params_update=params_update, active_topic=topic, dialog_focus=_dialog_focus_for_current(plan, state))
...
def _runtime_response_composer_meta(meta: dict[str, Any], *, mode: str, published: bool, elapsed_ms: int) -> dict[str, Any]:
    ...
    if isinstance(meta.get("semantic_diagnostics"), list):
        out["semantic_diagnostics"] = [dict(item) for item in meta["semantic_diagnostics"][:2] if isinstance(item, dict)]
