...
"""Safe prompt identity helpers.

Only prompt source labels and SHA-256 identities are exposed.  Prompt bodies,
payloads and model responses must never be returned by this module.
"""
...
def identity_from_path(stage: Any, source: Any, path: str | Path | None = None, usage: str = "invoked") -> dict[str, Any]:
    ...
    return identity_from_text(stage, source, text, usage=usage)
...
def _safe_stage(value: Any) -> str:
    ...
    if not _SAFE_STAGE_RE.fullmatch(text):
        return "unknown"
    return text[:80]
...
def _safe_source(value: Any) -> str:
    ...
    if any(part in _FORBIDDEN_SOURCE_PARTS for part in Path(text).parts):
        return "unknown"
    lowered = text.lower()
    if any(word in lowered for word in ("token", "secret", "payload", "response_body", "prompt_body")):
        return "unknown"
    return text[:160]
