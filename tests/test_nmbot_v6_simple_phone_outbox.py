...
import scripts.nmbot_crm_outbox as crm_outbox
...
def test_test_profile_cannot_enable_crm_even_when_control_says_enabled(monkeypatch):
    monkeypatch.setenv("NMBOT_CALLBACK_CRM_CONTROL_FILE", "/private/control.json")
    import scripts.nmbot_callback_crm_control as control
    monkeypatch.setattr(control, "read_control", lambda *_args, **_kwargs: True)

    assert crm_outbox._crm_enabled_for_contour("TEST") is False
    assert crm_outbox._crm_enabled_for_contour("UNKNOWN") is False
    assert crm_outbox._crm_enabled_for_contour("PROD") is True
