...
def _runtime_response_composer_meta(meta: dict[str, Any], *, mode: str, published: bool, elapsed_ms: int) -> dict[str, Any]:
    ...
    out = {
        ...
        "attempt_summaries": [dict(item) for item in meta.get("attempt_summaries", ())[:1]] if isinstance(meta.get("attempt_summaries"), (list, tuple)) else [],
    }
...
def _selected_live_fact_pending(plan: TurnPlan, execution: ExecutionResult) -> str | None:
    ...
    if not requested or str(execution.error_code or "").startswith("selected_enrichment_"):
        return None
