...
def _technical_operator_fallback_plan() -> ResponsePlan:
    return ResponsePlan(
        acknowledgement="Сейчас не могу надёжно проверить нужную информацию, поэтому не буду гадать.",
        caveat="Лучше передать вопрос специалисту, он сможет проверить актуальность вручную.",
...
def build_response_plan(
    *,
    ...
    if not execution.ok:
        return _technical_operator_fallback_plan()
    ...
    if stage == Stage.SELECTED_OBJECT and execution.selected:
        ...
        if _is_selected_financing_request(plan, requested):
            capability_status = str(execution.bridge_status or "")
            if capability_status in {"selected_capability_evidence_complete", "selected_capability_evidence_partial"}:
                terms = str(execution.selected.mortgage_terms or "").strip()
                partial = capability_status.endswith("partial")
                return ResponsePlan(
                    acknowledgement=(
                        f"По {_display_name(execution.selected.name)} подтверждены условия: {terms}."
                        + (" Часть условий пока не подтверждена — назвала только то, что нашла в проверенных данных." if partial else "")
                    ),
                    cards=(),
                    final_question="Хотите проверить ещё что-нибудь по этому ЖК?",
                    answer_kind="selected_mortgage_verified_partial" if partial else "selected_mortgage_verified",
                    viewpoint="financing",
                )
            if capability_status.startswith("selected_capability_"):
                return ResponsePlan(
                    acknowledgement=f"По {_display_name(execution.selected.name)} сейчас не удалось подтвердить условия ипотеки. Не буду называть неподтверждённые цифры.",
                    cards=(), final_question="Передать запрос оператору?", operator_prompt=True,
                    answer_kind="selected_mortgage_unconfirmed", viewpoint="financing",
                )
            non_finance_requested = tuple(
                fact for fact in requested
                if str(fact).strip().lower() != "mortgage_terms"
            )
            available_non_finance = split_requested_facts(
                non_finance_requested, execution.selected, fresh_facts=execution.fresh_facts
            ).available
            fact_answer = _selected_fact_acknowledgement(
                execution.selected,
                replace(plan, requested_facts=available_non_finance, facts_needed=available_non_finance),
                fresh_facts=execution.fresh_facts,
            ) if available_non_finance else None
            return ResponsePlan(
                acknowledgement=(
                    (fact_answer + " " if fact_answer else "")
                    + f"По {_display_name(execution.selected.name)} точные условия зависят от банка, "
                    "застройщика и конкретной квартиры; условия ипотеки сначала подтвержу в проверенных данных."
                ),
                cards=(),
        ...
        if requested and str(execution.error_code or "").startswith("selected_enrichment_"):
            return ResponsePlan(
                acknowledgement=(
                    f"Сейчас не могу надёжно проверить {_selected_fact_topic(requested)} для "
                    f"{_display_name(execution.selected.name)}. Чтобы не выдумывать, лучше уточнить это у специалиста."
                ),
                cards=(),
                final_question="Передать оператору запрос?",
                operator_prompt=True,
    ...
    if stage == Stage.CURRENT_OPTIONS and execution.comparison_cards:
        status = str(execution.comparison_metadata.get("status") or execution.error_code or "").strip()
        caveat = None
        if status in {"partial_enrichment_failed", "all_enrichment_failed"}:
            caveat = "Часть свежих деталей не подтвердилась, поэтому сравниваю только по тем данным, которые уже есть в карточках."
        return ResponsePlan(
            acknowledgement="Сравню только эти два варианта по подтверждённым данным, без выбора «лучшего» за вас.",
            result_summary="Третий вариант из текущей подборки сюда не добавляю.",
            cards=tuple(execution.comparison_cards[:2]),
            caveat=caveat,
            final_question="По какому из этих двух ЖК проверить квартиры или условия подробнее?",
            answer_kind="compare_current_pair",
            viewpoint=plan.intent or state.active_topic,
        )
    ...
    for fact in dict.fromkeys(requested):
        if fact == "mortgage_terms":
            if card.mortgage_terms:
                ...
                parts.append("Точных условий по ипотеке сейчас нет.")
...
def _is_selected_financing_request(plan: TurnPlan, requested: tuple[str, ...]) -> bool:
    return (
        "mortgage_terms" in {str(fact).strip().lower() for fact in requested}
        or str(plan.intent or "").strip().lower() in {"mortgage", "financing"}
    )
