...
from nmbot_v2.contracts import ExecutableTurn, IntentGoal, OptionCard, PendingAction, ResponseBrief, SafeTurnContext, SearchResult, SelectedEntity, SemanticPlan, Stage, StateDelta, TurnAction
...
def test_runtime_demotes_missing_or_violated_hard_facts_to_structured_near_cards() -> None:
    parsed = {
        "facts": [
            {"id": "future", "name": "ЖК Будущий", "ready": "строится", "finishing": "без отделки"},
            {"id": "unknown", "name": "ЖК Неизвестный"},
            {"id": "lot", "name": "ЖК Без лота", "rooms": [2]},
        ],
        "near": [],
    }
    validation = {
        "errors": [
            "fact_0_violates_hard:ready",
            "fact_0_violates_hard:finishing",
            "fact_1_missing_hard_evidence:ready",
            "fact_2_violates_lot_hard:rooms",
        ],
    }

    moved = runtime_adapter_mod._move_hard_invalid_facts_to_near(parsed, validation)
    ...
    assert moved["near"] == [
        {
            ...
            "is_near": True,
            "why_close": "другой срок готовности; другой вариант отделки",
            "differences": ["другой срок готовности", "другой вариант отделки"],
        },
        {
            "id": "unknown",
            "name": "ЖК Неизвестный",
            "is_near": True,
            "why_close": "готовность дома не подтверждена",
            "differences": ["готовность дома не подтверждена"],
        },
        {
            "id": "lot",
            "name": "ЖК Без лота",
            "rooms": [2],
            "is_near": True,
            "why_close": "другая комнатность",
            "differences": ["другая комнатность"],
        },
    ]


def test_runtime_keeps_valid_facts_and_does_not_duplicate_demoted_near_card() -> None:
    parsed = {
        "facts": [
            {"id": "valid", "name": "ЖК Точный", "ready": "сдан"},
            {"id": "near", "name": "ЖК Почти", "ready": "строится"},
        ],
        "near": [{"id": "near", "name": "ЖК Почти", "is_near": True, "why_close": "другой срок готовности", "differences": ["ready"]}],
    }

    moved = runtime_adapter_mod._move_hard_invalid_facts_to_near(parsed, {"errors": ["fact_1_violates_hard:ready"]})

    assert [card["name"] for card in moved["facts"]] == ["ЖК Точный"]
    assert moved["near"] == parsed["near"]
...
def test_v2_search_adapter_demotes_missing_hard_evidence_to_safe_near_card(monkeypatch):
    async def scenario() -> None:
        env(monkeypatch, version="v2")
        ...
        assert result.facts == ()
        assert [card.name for card in result.near] == ["ЖК Без комнат"]
        assert [card.why_close for card in result.near] == ["комнатность не подтверждена"]
    ...
    async def scenario() -> None:
        ...
        assert envelope == {
            ...
            "context": {
                ...
                "viewpoint": "financing",
                "requested_facts_policy": "pending_action",
                "default_requested_facts": ("mortgage_terms",),
...
def test_v2_response_composer_trace_allows_only_safe_attempt_diagnostic() -> None:
    trace = _safe_response_composer_trace(
        {
            ...
            "semantic_diagnostics": [
                {"stage": "formatter", "categories": ["numeric_price_not_in_canonical", "numeric_transit_not_in_canonical", "sensitive_claim", "raw secret"], "matched_text": "99 млн", "position": 12},
    ...
    assert trace["semantic_diagnostics"] == [
        {"stage": "formatter", "categories": ["numeric_price_not_in_canonical", "numeric_transit_not_in_canonical", "sensitive_claim"]}
    ]
    dumped = json.dumps(trace, ensure_ascii=False)
...
def test_canonical_v2_round_trip_preserves_selected_capability_without_leaking_identity() -> None:
    selected = SelectedEntity("residential_complex", 42, "Лучи")
    ...
    restored = _legacy_to_v2_state(runtime_adapter_mod._canonical_v2_envelope(first_turn))
    assert restored.selected_entity == selected
    assert restored.pending_action == action
    planner_state = _v2_to_planner_legacy_state(restored)
    assert "42" not in json.dumps(planner_state, ensure_ascii=False)
    assert "verify-mortgage-42" not in json.dumps(planner_state, ensure_ascii=False)

    accepted = derive_transition(SemanticPlan(operation="freeform", followup_outcome="accept"), restored)
    assert accepted.action == TurnAction.ANSWER_SELECTED_OPTION
    assert accepted.action != TurnAction.ACCEPT_OPERATOR

    second_turn = _legacy_to_v2_state(runtime_adapter_mod._canonical_v2_envelope(restored))
    assert second_turn.selected_entity == selected
    assert second_turn.pending_action == action

    malformed = runtime_adapter_mod._canonical_v2_envelope(first_turn)
    malformed["nmbot_v2"]["selected_entity"] = {"entity_type": "unknown", "entity_id": 42, "display_name": "Лучи"}
    malformed["nmbot_v2"]["pending_action"] = {"action_type": "verify_selected_facts", "fact_keys": ["mortgage_terms"], "entity_type": "residential_complex", "entity_id": 42, "status": "pending", "idempotency_key": "x", "unknown": True}
    dropped = _legacy_to_v2_state(malformed)
    assert dropped.selected_entity is None
    assert dropped.pending_action is None
