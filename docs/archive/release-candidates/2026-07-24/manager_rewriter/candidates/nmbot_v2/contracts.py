...
class SearchResult:
    ...
    def shortlist(self, limit: int = 3) -> tuple[OptionCard, ...]:
        ...
        Broad search should show exact facts first, then near alternatives, with
        stable name-based deduplication.  The card itself is not modified, so a
        near card keeps its ``is_near`` marker for honest client wording.
        """
        ...
        for card in (*self.facts, *self.near):
