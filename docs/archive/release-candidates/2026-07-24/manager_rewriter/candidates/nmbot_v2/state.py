...
def merge_enriched_card_cache(entries: tuple[EnrichedCardCacheEntry, ...], entry: EnrichedCardCacheEntry, *, limit: int = 12) -> tuple[EnrichedCardCacheEntry, ...]:
    retained = [item for item in entries if item.identity != entry.identity]
    return tuple([entry, *retained][:max(1, limit)])


def merge_enriched_card_cache_entries(entries: tuple[EnrichedCardCacheEntry, ...], additions: tuple[EnrichedCardCacheEntry, ...]) -> tuple[EnrichedCardCacheEntry, ...]:
    merged = entries
