#!/usr/bin/env python3
"""Build label-blind Qapairs navigation/context-gate predictions."""
...
DEFAULT_INPUTS = PROJECT_ROOT / "tests" / "fixtures" / "qapairs_context_acceptance_inputs_v1.json"
DEFAULT_OUTPUT = Path("/tmp/opencode/qapairs_context_acceptance_predictions_v1.json")
MAX_SOURCES = 2
MAX_LINES = 80
MAX_CHARS = 8000
...
def predict_case(case: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        nav = navigation.navigate(code, project_id="qapairs")
...
def generate(input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": 1,
        "project_id": "qapairs",
        "harness": "qapairs_context_acceptance_v1",
