...
def test_financing_replaces_generic_selected_intro_and_uses_registry() -> None:
    ...
    assert transition_for_reply("financing_consent", "accept").action == TurnAction.ACCEPT_OPERATOR


def test_selected_compound_finishing_metro_and_mortgage_covers_every_requested_fact() -> None:
    card = OptionCard(name="Бусиновский парк", finishing="с отделкой", metro="Ховрино", ready="сдан")
    ...
    assert "Точных условий по ипотеке сейчас нет" in turn.response_text
