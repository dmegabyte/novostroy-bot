...
from nmbot_v2.contracts import ExecutableTurn, ExecutionResult, IntentGoal, IntentPlanV3, OptionCard, PendingAction, RetrySearchContext, SafeTurnContext, SearchResult, SelectedEntity, SemanticPlan, StateDelta, TurnAction, TurnPlan, TurnResult, to_jsonable
...
class _OvermindSearchAdapter:
    ...
    async def _search_once(self, client: Any, contract: Any, *, prompt: str, validation_contract: Any | None = None) -> tuple[SearchResult, tuple[dict[str, Any], ...]]:
        ...
        parsed = _move_hard_invalid_facts_to_near(parsed, validation)
    ...
    async def verify_selected_capability(self, option: OptionCard, request: Any) -> tuple[OptionCard, Any, dict[str, Any]]:
        client = self.app.get("overmind_client") if hasattr(self.app, "get") else None
        if client is None or not hasattr(client, "_run_gateway_request"):
            from nmbot_v2.evidence_resolver import EvidenceResult, EvidenceStatus
            return option, EvidenceResult(EvidenceStatus.EVIDENCE_EMPTY), {"status": "selected_capability_transport"}
        async def gateway(payload: dict[str, Any]) -> tuple[Any, dict[str, Any]]:
            return await _run_v2_low_level_gateway(client, payload)
        return await fetch_selected_capability(option, request, gateway, timeout=max(0.2, _safe_float_env("NMBOT_V2_ENRICHMENT_ITEM_TIMEOUT", 20.0)), model=SEARCH_MODEL)
...
_HARD_NEAR_REASONS = {
    ...
    # Keep the established candidate-first wording for location cards.
    "location": ("локация не подтверждена", "локация отличается от запроса"),
...
def _move_hard_invalid_facts_to_near(parsed: Any, validation: Mapping[str, Any]) -> Any:
    """Demote every fact rejected by a hard-constraint check to safe ``near``.

    Search-contract validation deliberately retains invalid facts for diagnostics.
    The runtime is the client-visible owner, so it must turn those validation
    codes into fixed, structured near explanations before cards are rendered.
    """
    ...
    if not facts:
        return parsed
    move_reasons: dict[int, dict[str, tuple[int, str]]] = {}
    for error in validation.get("errors") if isinstance(validation.get("errors"), list) else []:
        match = re.fullmatch(r"fact_(\d+)_(missing_hard_evidence|violates_hard|violates_lot_hard):([a-z_]+)", str(error or ""))
        if not match:
            continue
        field = match.group(3)
        labels = _HARD_NEAR_REASONS.get(field)
        if labels is None:
            continue
        kind = match.group(2)
        priority = 0 if kind == "missing_hard_evidence" else 1
        reason = labels[0] if priority == 0 else labels[1]
        index = int(match.group(1))
        reasons_for_card = move_reasons.setdefault(index, {})
        previous = reasons_for_card.get(field)
        if previous is None or priority < previous[0]:
            reasons_for_card[field] = (priority, reason)
    if not move_reasons:
        return parsed
    ...
    for index in sorted(move_reasons):
        ...
        if not isinstance(card, dict):
            continue
        moved = dict(card)
        labels = [reason for _priority, reason in move_reasons[index].values()][:2]
        moved.update({
            "is_near": True,
            "why_close": "; ".join(labels),
            "differences": labels,
        })
...
def _move_location_invalid_facts_to_near(parsed: Any, validation: Mapping[str, Any]) -> Any:
    """Backward-compatible alias for callers of the former location-only helper."""
    return _move_hard_invalid_facts_to_near(parsed, validation)
...
def _inherit_selected_scope(plan: TurnPlan, state: ConversationState) -> TurnPlan:
    ...
    # A consent reply has no object/facts of its own. Restore them only from
    # the structured offer, never from the free-form word "да".
    ...
    if state.pending_followup == FINANCING_CONSENT_FOLLOWUP and action and action.status == "pending":
        ...
        requested = tuple(action.fact_keys)
        if isinstance(plan, ExecutableTurn):
            return replace(plan, selected_option_name=selected or plan.selected_option_name, reference=selected or plan.reference, scope="one", requested_facts=requested, facts_needed=requested, intent="mortgage", followup_outcome=plan.followup_outcome)
        return replace(plan, operation="select_option", selected_option_name=selected or plan.selected_option_name, reference=selected or plan.reference, scope="one", requested_facts=requested, facts_needed=requested, intent="mortgage", followup_outcome=plan.followup_outcome)
    if isinstance(plan, ExecutableTurn):
...
def _pending_scenario_for_planner(state: ConversationState) -> dict[str, Any] | None:
    ...
    if has_selected:
        scope = "one"
    elif scope_policy == "selected_required":
        selected = ""
        scope = "one"
    else:
        selected = ""
    ...
    facts_policy = str(planner_context.get("requested_facts_policy") or "")
    if facts_policy == "dialog_focus":
        requested = state.dialog_focus.last_requested_facts
        envelope["context"]["requested_facts"] = [fact for fact in requested if fact]
    elif facts_policy == "pending_action" and state.pending_action:
        envelope["context"]["requested_facts"] = list(state.pending_action.fact_keys)
    elif isinstance(planner_context.get("default_requested_facts"), (list, tuple)):
        envelope["context"]["requested_facts"] = list(planner_context["default_requested_facts"])
...
def _safe_v2_state_dict(data: dict[str, Any]) -> dict[str, Any]:
    allowed = {
        ...
        "comparison_scope_option_names",
        "last_search", "operator_offered", "operator_declined", "active_topic", "dialog_focus", "selected_enriched", "selected_entity", "pending_action", "enriched_card_cache",
    ...
    for key, value in data.items():
        ...
        if key == "selected_entity":
            try:
                safe[key] = SelectedEntity.from_dict(value).to_dict()
            except Exception:
                continue
        elif key == "pending_action":
            try:
                safe[key] = PendingAction.from_dict(value).to_dict()
            except Exception:
                continue
        else:
            safe[key] = _safe_nested(value)
    return safe
...
def _safe_runtime_summary_trace(value: Any) -> dict[str, Any]:
    ...
    capability_outcome = _safe_capability_outcome(value.get("capability_outcome"))
...
def _safe_capability_outcome(value: Any) -> dict[str, Any] | None:
    if not isinstance(value, dict) or value.get("status") not in _SAFE_CAPABILITY_STATUSES:
        return None
    facts = value.get("requested_facts") if isinstance(value.get("requested_facts"), list) else []
    status = str(value["status"])
    evidence = str(value.get("evidence_status") or "")
    ...
    elif evidence not in _SAFE_CAPABILITY_EVIDENCE: evidence = "unknown"
    out: dict[str, Any] = {
        "requested_facts": list(dict.fromkeys(str(item) for item in facts if str(item) in _SAFE_CAPABILITY_FACTS))[:8],
        "status": status, "request_count": _bounded_int(value.get("request_count"), 0, 1),
        "evidence_status": evidence, "accepted_count": _bounded_int(value.get("accepted_count"), 0, 10),
    ...
    if value.get("parse_class") in {"structured", "invalid"}:
        out["parse_class"] = value["parse_class"]
    return out
...
def _safe_call_counts(value: Any) -> dict[str, int]:
    ...
    return {
        ...
        "selected_capability": _bounded_int(counts.get("selected_capability"), 0, 1),
...
def _safe_semantic_diagnostics(meta: dict[str, Any], *, reason: str | None, validation_stage: str | None) -> list[dict[str, Any]]:
    ...
    for item in raw_items[:2]:
        ...
        if categories:
            out.append({"stage": stage, "categories": list(dict.fromkeys(categories))[:3]})
    return out
