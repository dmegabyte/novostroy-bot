...
from nmbot_v2.contracts import ExecutableTurn, ExecutionResult, IntentGoal, IntentPlanV3, OptionCard, SafeTurnContext, SearchResult, Stage, TurnAction
...
import followup_intent_classifier as classifier
...
def test_explicit_pair_then_deictic_compare_reuses_exact_pair_without_search() -> None:
    search = PairSearch()
    first = TurnProcessor(planner=Planner(_pair_turn("ЖК Первый", "ЖК Третий")), search_service=search).process(
        SafeTurnContext("u", "сравни первый и третий"), _state()
    )
    state = ConversationState.from_dict(first.state)
    raw = classifier._normalize_v3_deictic_comparison_scope(
        "Сравни их",
        {
            "schema_version": 3,
    ...
    second_plan = compile_executable_turn_v3({key: value for key, value in raw.items() if key in IntentPlanV3.__dataclass_fields__}, state)
    second = TurnProcessor(planner=Planner(second_plan), search_service=search).process(SafeTurnContext("u", "Сравни их"), state)

    assert state.comparison_scope_option_names == ("ЖК Первый", "ЖК Третий")
    assert second_plan.comparison_option_names == ("ЖК Первый", "ЖК Третий")
    assert second.execution.comparison_cards and [card.name for card in second.execution.comparison_cards] == ["ЖК Первый", "ЖК Третий"]
    assert "ЖК Второй" not in second.response_text
    assert search.search_calls == 0


def test_generic_deictic_scope_normalization_does_not_change_non_deictic_comparison() -> None:
    raw = {"goal": "compare_current", "comparison_option_names": []}
    state = _state().to_dict() | {"comparison_scope_option_names": ["ЖК Первый", "ЖК Третий"]}

    assert classifier._normalize_v3_deictic_comparison_scope("сравни варианты", raw, state) == raw
