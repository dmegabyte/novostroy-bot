...
from .contracts import DialogFocus, OptionCard, PendingAction, RetrySearchContext, SearchResult, SelectedEntity, StateDelta, to_jsonable
...
class ConversationState:
    ...
    # Bounded canonical scope of the last explicit two-option comparison.
    # This is state, not a transcript: only exact current option names belong here.
    comparison_scope_option_names: tuple[str, str] = ()
    ...
    def from_dict(cls, data: Mapping[str, Any] | None) -> "ConversationState":
        ...
        return cls(
            ...
            comparison_scope_option_names=_comparison_scope_option_names(data.get("comparison_scope_option_names")),
            ...
            selected_entity=SelectedEntity.from_dict(data["selected_entity"]) if data.get("selected_entity") else None,
...
def apply_state_delta(state: ConversationState, delta: StateDelta, *, accepted: bool = True) -> ConversationState:
    ...
    next_state = replace(
        ...
        comparison_scope_option_names=keep_or(
            delta.comparison_scope_option_names,
            state.comparison_scope_option_names,
        ...
        selected_entity=keep_or(delta.selected_entity, state.selected_entity, "selected_entity"),
        pending_action=keep_or(delta.pending_action, state.pending_action, "pending_action"),
    ...
    # Keep action ownership in one place.  Local import avoids the state ↔
    # lifecycle module import cycle while preserving the reducer as the only
    # persistence boundary.
    from .pending_action import reconcile_pending_action_lifecycle

    return reconcile_pending_action_lifecycle(
        state,
        next_state,
        pending_followup_replaced=(
            "pending_followup" in delta.clear_fields
            or delta.replaces_pending_offer
            or (delta.pending_followup is not None and delta.pending_followup != state.pending_followup)
        ),
        fresh_pending_action=delta.pending_action is not None,
    )
...
def _comparison_scope_option_names(value: Any) -> tuple[str, str]:
    """Restore only a complete, bounded pair; corrupt persisted values fail closed."""
