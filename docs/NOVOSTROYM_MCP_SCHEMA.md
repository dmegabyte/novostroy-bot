# MCP novostroym — канонический справочник базы и operational contract

Правила сборки model+MCP search-запроса, evidence mapping и результаты
контролируемой prompt-матрицы находятся в
`docs/NMBOT_V2_MCP_PROMPT_BUILD_RULES.md`. Этот документ остаётся источником
wire-схемы; prompt assembly, probes и runtime не должны держать собственные
копии schema allowlist.

Источник: структура MCP/БД `novostroym`, переданная пользователем и актуализированная
2026-07-30. Этот файл — видимый справочник рядом с
[`MCP_APARTMENT_REQUEST_RULES.md`](MCP_APARTMENT_REQUEST_RULES.md), а не копия
неподтверждённого `tools/list` ответа.

## 0. Подтверждённые operational notes

- `novos.state = 2` означает активный ЖК.
- Для объявления `ads.state = 2` означает активную запись, а `ads.status = 2` —
  объявление в продаже. `status = 1` означает бронь.
- Связь лота с ЖК: `ads.id → ads.novos_id → novos.id/name`; корпус связан как
  `ads.house_id → house.id → house.novos_id`.
- В selected-enrichment структурированный результат проходит через `ads` и
  нормализуется в максимум два typed `LotExample`. Наличие можно подтверждать
  только по лоту с ID и активным/in-sale статусом; текст модели или число
  `apartment_inventory` доказательством не являются.
- TEST enrichment task `2451943` вернул два структурированных лота с ID,
  комнатностью, площадью, полной ценой и `status = 2`. Сырые ответы модели/MCP
  в документацию не копируются.
- Полный input JSON Schema/handler прямого `SQL_QUERY_TOOL` в проектном adapter
  не зафиксирован. Ниже описаны подтверждённые таблицы и SQL-шаблоны, но нельзя
  выдумывать новый `tools/call` payload или считать этот документ заменой
  фактического runtime adapter.

### V6: повторный exact `get_flat_info`, 2026-08-10

В активном V6 не требуется новый transport для запроса дополнительной информации
по уже выбранному ЖК. Проверенный runtime-маршрут повторно вызывает логический
MCP tool `novostroym/get_flat_info` через тот же gateway-контур. TEST task
`gateway-2512265` для «Люблинского парка» с комнатностью `rooms="2"` завершился
успешно и вернул расширенные project-level факты: диапазон площади и цены для
двухкомнатного формата, корпуса/срок, отделку, метро/МЦД и инфраструктуру.

Контракт follow-up после вопроса «Показать больше информации?»:

1. сохранить точный выбранный ЖК и effective constraints первого поиска;
2. нормализовать wire-значение комнатности: строка `"2"` становится typed
   `rooms=2` (без подстрочного сопоставления сложных значений);
3. на подтверждение клиента повторить exact `get_flat_info` с canonical ЖК,
   `search_mode=named_object`, `count=1`, комнатностью и расширенным allowlist
   полей (`developer`, location/metro, infrastructure, `ready`, `finishing`,
   `area`, project price, `house`, `ads`, `apartment_types`, `lot_examples`);
4. не переключать этот сценарий на отдельный direct gateway probe и не делать
   вывод о необходимости нового transport только потому, что такой probe не
   вернул task ref: он не повторяет штатный V6 webhook-маршрут;
5. отсутствие `ads`/typed `LotExample` не мешает показать подтверждённую
   расширенную информацию о ЖК, но запрещает обещать конкретные планировки,
   наличие или доступные квартиры. Эти утверждения по-прежнему требуют
   `ads.id`, `ads.state=2`, `ads.status=2`.

Safe task status и model-authored projection подтверждают поведение V6-контура,
но не заменяют raw backend trace. Поэтому документ фиксирует проверенную
клиентскую capability `get_flat_info`, а не выдумывает внутренний SQL/tool payload.

### Матрица доказательств

| Утверждение | Минимальное structured evidence | Канонический runtime-объект | Можно сообщать клиенту |
|---|---|---|---|
| ЖК активен | `novos.state = 2` | `OptionCard` | Да |
| Лот активен и в продаже | конкретный `ads.id`, `ads.state = 2`, `ads.status = 2` | `LotExample` | Да, как каталожный лот на момент запроса |
| Лот забронирован | конкретный `ads.id`, `ads.status = 1` | `LotExample` | Да, только как бронь; не как доступный лот |
| Наличие квартир | typed `LotExample` с ID и активным/in-sale статусом | `apartment_inventory` + `lot_examples` | Да, с оговоркой о моменте запроса |
| Бронь гарантирует сделку | такого evidence нет | — | Нет |
| Текст модели сообщает число квартир | structured MCP evidence отсутствует | — | Нет |
| Gateway task завершился | safe task/status metadata | trace | Это не доказывает MCP-вызов или его результат |

Эта матрица разделяет справочную структуру базы и фактическое доказательство,
которое дошло до runtime. Наличие поля в базе не означает, что поле доступно
через текущий `novostroym/get_flat_info` wire-контракт.

### Актуализация wire-формата MCP, 2026-07-19

Помимо схемы базы, search-agent обязан учитывать реальные формы значений,
которые приходят в ответе MCP:

- `rooms` может быть числом, строкой (`"2-комнатные"`, `"1, 2, 3, студии"`)
  или списком. Перед проверкой комнатности значение нужно нормализовать в
  отдельные токены; поиск по подстроке запрещён (`1` не должен совпадать с
  `10`). Текстовые поля вроде `description` не являются доказательством
  комнатности.
- `delivered` приходит не только как boolean, но и как числовой флаг:
  `1` — дом сдан, `0` — не сдан. Значения `ready/state/status` могут содержать
  человекочитаемые формы вроде `сдан`, `дом сдан`, `готов`, `строится`.
  Будущий год или квартал сам по себе не означает, что дом сдан.
- `district` — региональный код `msk|mo|newmsk`; `location` может быть
  строкой или списком локаций. Нельзя проверять список через операции,
  требующие hashable-значение, и нельзя подменять `location` кодом региона.
- `facts[]`, `near[]`, `missing`, `params` и служебные diagnostics — разные
  слои. Diagnostics (`mcp_tool`, viewpoint, priorities, relaxation audit и
  ignored preferences) достраиваются runtime из входного запроса; модель не
  является источником этих служебных полей.

Эти правила закреплены в `prompts/v2_search_mcp.txt`,
`nmbot_v2/search_contract.py` и `tests/fixtures/v2_search_mcp_contract.json`.
...
### Статус прямого вызова

Структура базы и SQL-шаблоны ниже — reference для анализа и будущего adapter.
Полная входная JSON Schema, transport и handler `sql_query` в текущем проекте
не подтверждены. Поэтому до появления реального adapter/fixture запрещено:

- собирать или отправлять прямой `tools/call` payload;
- считать `query`, `query_base64` или иной параметр доступным только по этому
  документу;
- заменять существующий `novostroym/get_flat_info` exact-enrichment прямым SQL.

Чтобы снять это ограничение, нужен один легитимный источник: фактический
`tools/list`/adapter contract либо fixture с реальным request/response. После
этого schema, adapter и tests обновляются одним изменением.
