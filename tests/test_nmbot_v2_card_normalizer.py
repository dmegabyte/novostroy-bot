...
from nmbot_v2.contracts import ExecutionResult, ResponsePlan, SemanticPlan, Stage, StateDelta
...
def test_root_mcp_id_is_internal_residential_complex_identity_only() -> None:
    card = normalize_card({"id": 42, "name": "Лучи", "type_object": "novo", "mortgage_calc": {"id": 99}, "ads": [{"id": 7}]})
    assert (card.entity_type, card.entity_id) == ("residential_complex", 42)
    assert normalize_card({"name": "Без root", "mortgage_calc": {"id": 99}, "ads": [{"id": 7}]}).entity_id is None

    for invalid in (True, {}, [], "", "x" * 129):
        rejected = normalize_card({"id": invalid, "name": "Лучи"})
        assert rejected.entity_id is None and rejected.entity_type is None
    incompatible = normalize_card({"id": 42, "name": "Лучи", "type_object": "apartment"})
    assert incompatible.entity_id is None and incompatible.entity_type is None

    text = render_response(ResponsePlan(acknowledgement="Нашла.", cards=(card,), final_question="Смотрим?"))
    assert "42" not in text and "entity_id" not in text
...
def test_mortgage_program_list_is_normalized_separately_from_discount() -> None:
    card = normalize_card({
        ...
        "discount": "скидка 3%",
        "mortgage_calc": [
            {"novos_id": 9, "state": 2, "min_percent": 6.0, "min_fee": 20},
            {"novos_id": 9, "state": 2, "min_percent": 6.2},
        ],
    })
