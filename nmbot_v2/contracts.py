...
from .vocabulary import ENTITY_TYPES, FACT_KEY_SET, PENDING_ACTION_STATUSES, PENDING_ACTION_TYPES


JsonDict = dict[str, Any]
INTENT_PLAN_V3_FOLLOWUP_OUTCOMES = frozenset({"accept", "decline", "ask_or_clarify", "unexpected", "resume_contact"})
...
def _bounded_scalar(value: Any, field_name: str, *, maximum: int = 200, allow_int: bool = False) -> str | int:
    if isinstance(value, bool) or isinstance(value, (Mapping, list, tuple, set)) or value is None:
        raise ValueError(f"{field_name} must be a bounded scalar")
    if allow_int and isinstance(value, int):
        return value
    if not isinstance(value, str):
        raise ValueError(f"{field_name} must be a string{' or integer' if allow_int else ''}")
...
class IntentPlanV3:
    ...
    def __post_init__(self) -> None:
        ...
        followup_outcome = _optional_str(self.followup_outcome)
        if followup_outcome is not None and followup_outcome not in INTENT_PLAN_V3_FOLLOWUP_OUTCOMES:
            raise ValueError("invalid followup_outcome")
        object.__setattr__(self, "followup_outcome", followup_outcome)
    ...
    def to_dict(self) -> JsonDict:
        return {
            ...
            "followup_outcome": self.followup_outcome,
...
class SelectedEntity:
    ...
    def __post_init__(self) -> None:
        entity_type = _bounded_scalar(self.entity_type, "entity_type", maximum=64)
        if entity_type not in ENTITY_TYPES:
            raise ValueError("unknown entity_type")
        object.__setattr__(self, "entity_type", entity_type)
        object.__setattr__(self, "entity_id", _bounded_scalar(self.entity_id, "entity_id", maximum=128, allow_int=True))
        object.__setattr__(self, "display_name", _bounded_scalar(self.display_name, "display_name", maximum=200))

    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> "SelectedEntity":
        if not isinstance(data, Mapping) or set(data) != set(cls.__dataclass_fields__):
            raise ValueError("SelectedEntity must contain only entity_type, entity_id, display_name")
        return cls(**dict(data))

    def to_dict(self) -> JsonDict:
        return {"entity_type": self.entity_type, "entity_id": self.entity_id, "display_name": self.display_name}
...
class PendingAction:
    ...
    def __post_init__(self) -> None:
        action_type = _bounded_scalar(self.action_type, "action_type", maximum=64)
        status = _bounded_scalar(self.status, "status", maximum=32)
        if action_type not in PENDING_ACTION_TYPES or status not in PENDING_ACTION_STATUSES:
            raise ValueError("unknown pending action type or status")
        raw_facts = self.fact_keys if isinstance(self.fact_keys, (list, tuple, set)) else ()
        facts = tuple(dict.fromkeys(_bounded_scalar(item, "fact_key", maximum=64) for item in raw_facts))
        if not facts or any(fact not in FACT_KEY_SET for fact in facts):
            raise ValueError("unknown or empty pending action fact_keys")
        entity_type = _bounded_scalar(self.entity_type, "entity_type", maximum=64)
        if entity_type not in ENTITY_TYPES:
            raise ValueError("unknown entity_type")
        object.__setattr__(self, "action_type", action_type)
        object.__setattr__(self, "status", status)
        object.__setattr__(self, "fact_keys", facts)
        object.__setattr__(self, "entity_type", entity_type)
        object.__setattr__(self, "entity_id", _bounded_scalar(self.entity_id, "entity_id", maximum=128, allow_int=True))
        object.__setattr__(self, "idempotency_key", _bounded_scalar(self.idempotency_key, "idempotency_key", maximum=128))

    @classmethod
    def from_dict(cls, data: Mapping[str, Any]) -> "PendingAction":
        if not isinstance(data, Mapping) or set(data) != set(cls.__dataclass_fields__):
            raise ValueError("PendingAction has unknown or missing fields")
        return cls(**dict(data))

    def to_dict(self) -> JsonDict:
        return {"action_type": self.action_type, "fact_keys": list(self.fact_keys), "entity_type": self.entity_type, "entity_id": self.entity_id, "status": self.status, "idempotency_key": self.idempotency_key}
...
class StateDelta:
    ...
    # Distinguishes a fresh offer from a clarification that intentionally keeps
    # the same serialized follow-up key.
    replaces_pending_offer: bool = False
    selected_option_name: str | None = None
    comparison_scope_option_names: tuple[str, str] | None = None
