...
GENERATOR = ROOT / "scripts" / "qapairs_context_acceptance_generate.py"
SCORER = ROOT / "scripts" / "qapairs_context_acceptance_score.py"
INPUTS = ROOT / "tests" / "fixtures" / "qapairs_context_acceptance_inputs_v1.json"
LABELS = ROOT / "tests" / "fixtures" / "qapairs_context_acceptance_labels_v1.json"
...
def test_fixtures_are_label_blind_and_locked_labels_are_separate() -> None:
    ...
    assert len([row for row in labels if not row.get("abstain")]) == 12
...
def test_generator_source_has_no_label_access_or_owner_answers() -> None:
    source = GENERATOR.read_text(encoding="utf-8")
    labels = json.loads(LABELS.read_text(encoding="utf-8"))["cases"]
    assert "qapairs_context_acceptance_labels" not in source
    assert "expected" not in source.lower()
    for row in labels:
        if not row.get("abstain"):
            assert row["owner_path"] not in source
            assert row["symbol"] not in source


def test_harness_scripts_have_no_network_or_unbounded_write_imports() -> None:
...
def test_generate_then_score_acceptance_hard_pass(tmp_path: Path) -> None:
    ...
    assert result["metrics"]["positives_exact_owner_symbol"] == 12
    assert result["metrics"]["false_selections"] == 0
    assert result["metrics"]["false_abstentions"] == 0
    assert report.read_text(encoding="utf-8").startswith("# Qapairs context acceptance v1")
