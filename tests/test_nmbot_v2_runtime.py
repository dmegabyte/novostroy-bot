...
def test_runtime_summary_is_safe_aggregated_shape_for_empty_search() -> None:
    ...
    assert summary["call_counts"] == {"planner": 1, "search": 1, "selected_enrichment": 0, "selected_capability": 0, "gateway_attempts": 0}
...
def test_selected_mortgage_fact_check_uses_selected_object_financing_copy():
    ...
    assert "точные условия зависят от банка" in turn.response_text.lower()
    assert "Проверить условия по этому ЖК?" in turn.response_text
    assert "ставк" not in turn.response_text.lower()
    assert "первоначальн" not in turn.response_text.lower()
    assert "аккредит" not in turn.response_text.lower()
