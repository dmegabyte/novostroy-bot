...
def _journal_runtime_summary(result: dict[str, Any]) -> dict[str, Any] | None:
    ...
    capability_outcome = _journal_capability_outcome(summary.get("capability_outcome"))
    if capability_outcome:
        out["capability_outcome"] = capability_outcome
...
def _journal_capability_outcome(value: Any) -> dict[str, Any] | None:
    if not isinstance(value, dict) or value.get("status") not in _JOURNAL_CAPABILITY_STATUSES:
        return None
...
def _journal_counts(value: Any) -> dict[str, int]:
    ...
    return {
        ...
        "selected_capability": _journal_int(counts.get("selected_capability"), 0, 1),
