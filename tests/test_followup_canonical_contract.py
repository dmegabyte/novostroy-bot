...
def test_intent_plan_v3_followup_outcome_is_bounded_and_backwards_compatible() -> None:
    assert IntentPlanV3.from_dict({"schema_version": 3, "goal": "answer_current", "viewpoint": "life"}).followup_outcome is None
    assert IntentPlanV3.from_dict(intent_plan_v3_raw(followup_outcome="accept")).followup_outcome == "accept"
    with pytest.raises(ValueError, match="followup_outcome"):
        IntentPlanV3.from_dict(intent_plan_v3_raw(followup_outcome="yes"))
...
def test_intent_plan_v3_selected_null_defaults_are_safe() -> None:
    ...
    assert plan.followup_outcome is None
