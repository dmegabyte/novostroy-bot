...
def normalize_card(item: Mapping[str, Any], *, is_near: bool | None = None) -> OptionCard:
    ...
    normalized = {
        ...
        "entity_id": _root_entity_id(raw.get("id")) if _residential_complex_type(raw.get("type_object")) else None,
        "entity_type": "residential_complex" if _residential_complex_type(raw.get("type_object")) and _root_entity_id(raw.get("id")) is not None else None,
...
def _root_entity_id(value: Any) -> str | int | None:
    ...
    return text if text and len(text) <= MAX_ENTITY_ID_TEXT else None


def _residential_complex_type(value: Any) -> bool:
    if value is None:
        return True
    if not isinstance(value, str):
        return False
    return " ".join(value.casefold().split()) in {"residential_complex", "residential complex", "novo", "novos", "new_building", "новостройка", "жк"}
...
def _finance_text(raw: Mapping[str, Any]) -> str | None:
    bits: list[str] = []
    for mortgage in _verified_mortgage_calcs(raw):
...
def _mortgage_items(raw: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    return _verified_mortgage_calcs(raw)


def _verified_mortgage_calcs(raw: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    """Do not promote nested finance unless it proves this active root ЖК."""
    root_id = _root_entity_id(raw.get("id"))
    if root_id is None or str(raw.get("state")) != "2":
        return []
    value = raw.get("mortgage_calc")
    rows = [value] if isinstance(value, Mapping) else value if isinstance(value, list) else []
    programs = raw.get("mortgage")
    program_rows = [programs] if isinstance(programs, Mapping) else programs if isinstance(programs, list) else []
    active_programs = {str(item.get("id")) for item in program_rows if isinstance(item, Mapping) and str(item.get("state")) == "2"}
    return [
        row for row in rows
        if isinstance(row, Mapping)
        and str(row.get("novos_id")) == str(root_id)
        and str(row.get("state")) == "2"
        and (row.get("mortgage_id") in (None, "") or str(row.get("mortgage_id")) in active_programs)
    ][:5]
...
def _mortgage_term(raw: Mapping[str, Any]) -> str | int | None:
    for item in _mortgage_items(raw):
        for key in ("credit_month", "max_year", "years", "term", "term_years"):
