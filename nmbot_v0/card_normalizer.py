...
def _ready(raw: Mapping[str, Any]) -> str | None:
    for key in ("ready", "state", "status"):
        ...
        if value not in (None, ""):
            if key == "ready" and (
                value is True
                or (isinstance(value, (int, float)) and not isinstance(value, bool) and int(value) == 1)
                or (isinstance(value, str) and value.strip() == "1")
            ):
                return "сдан"
...
def _lot_example(ad: Mapping[str, Any], houses: Mapping[str, str]) -> LotExample | None:
    ...
    if not any(value is not None for value in (lot_id, rooms, area, floor, floors_total, full_price, renovation, status, house_id, house_name)):
        return None
    return LotExample(
        id=lot_id,
        rooms=rooms,
...
def _developer(value: Any) -> str | None:
    ...
    for item in values:
        if isinstance(item, Mapping):
            ...
            text = _text(item)
        if text and (
            _machine(text) in PROPERTY_CLASS
            or _machine(text) in {_machine(label) for label in PROPERTY_CLASS.values()}
        ):
            continue
...
def _infrastructure(raw: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    out = [label for key, label in names.items() if _binary_flag_enabled(raw.get(key))]
    values = raw.get("infrastructure")
    if isinstance(values, list):
        for item in values:
            if isinstance(item, Mapping):
...
def _finance_text(raw: Mapping[str, Any]) -> str | None:
    bits: list[str] = []
    if isinstance(raw.get("discount"), str) and raw.get("discount"):
        bits.append(str(raw["discount"]))
    mortgage = raw.get("mortgage_calc") if isinstance(raw.get("mortgage_calc"), Mapping) else raw.get("mortgage") if isinstance(raw.get("mortgage"), Mapping) else {}
