...
| Ambiguous `release status` / remembered TEST root | `python scripts/nmbot.py contour recon --contour <id>` | Requires an explicit registry contour and performs only SSH/loopback reads; traffic role remains unverified without correlated Jivo evidence | release owner TBD |
...
Source references: `docs/NMBOT_PROJECT_SIMPLIFICATION_PLAN.md:207-221,493-509,529-543,558-571`; `docs/NMBOT_OPERATIONS_MAP.md:3,23-27`; `docs/NMBOT_CONTEXT_PACKS.md`; `docs/NMBOT_RECIPE_OVERLAP.md`; `scripts/nmbot_context_pack.py`; `scripts/nmbot_recipe_overlap.py`; `scripts/nmbot_architecture_preflight.py:161-172`; `scripts/nmbot_diag.sh:5-9`; `scripts/nmbot_contour_recon.py`; `scripts/nmbot_release_preflight.py`; `scripts/nmbot_project_audit.py`; `scripts/nmbot.py`.
