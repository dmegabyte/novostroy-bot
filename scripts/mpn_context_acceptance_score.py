#!/usr/bin/env python3
"""Score MPN context acceptance predictions against locked labels."""
...
DEFAULT_PREDICTIONS = Path("/tmp/opencode/mpn_context_acceptance_predictions_v1.json")
DEFAULT_LABELS = PROJECT_ROOT / "tests" / "fixtures" / "mpn_context_acceptance_labels_v1.json"
DEFAULT_OUTPUT = Path("/tmp/opencode/mpn_context_acceptance_score_v1.json")
DEFAULT_REPORT = Path("/tmp/opencode/mpn_context_acceptance_report_v1.md")
EXPECTED_POSITIVES = 8
MAX_SOURCES = 2
MAX_LINES = 80
MAX_CHARS = 8000
...
def score(predictions_path: Path, labels_path: Path) -> dict[str, Any]:
    ...
    for cid, label in labels.items():
        ...
        if pred_abstain:
            ...
            continue
        if pred.get("owner_path") == label.get("owner_path") and pred.get("symbol") == label.get("symbol"):
            exact_positive += 1
        else:
        ...
        if pred.get("budget_status") == "context_budget_reached" and pred.get("stop_reason") == "context_budget_reached":
            context_budget_reached += 1
        ...
        cross_project_notebooks += int(pred.get("cross_project_notebooks") or 0)
        if not pred.get("local_read_only") or pred.get("production_proof") or pred.get("transitive_or_cross_root_claim"):
            unsafe_claims += 1
    ...
    if cross_project_notebooks != 0:
        failures.append("cross_project_notebooks_nonzero")
    metrics = {
        ...
        "positive_context_budget_reached": context_budget_reached,
        "max_sources_seen": max_sources_seen,
        "max_lines_seen": max_lines_seen,
        "max_chars_seen": max_chars_seen,
        "cross_project_notebooks": cross_project_notebooks,
        "unsafe_claims": unsafe_claims,
    }
    hard_pass = (
        exact_positive == EXPECTED_POSITIVES
