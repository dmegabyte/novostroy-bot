...
def test_model_followup_outcome_requires_active_reply_contract() -> None:
    without_pending = validate_intent_plan_v3(_raw(followup_outcome="accept"), _state())
    disallowed = validate_intent_plan_v3(
        _raw(followup_outcome="accept"), _state(pending_followup="contact_name")
    )

    assert without_pending.errors == ("followup_outcome_without_pending",)
    assert disallowed.errors == ("followup_outcome_not_allowed",)
