...
## 2026-07-09 — LLM planner выбирает вариант до operator-handoff 🧠

### Закрыта архитектурная дыра с placeholder `ЖК «выбранные варианты»`
- **Проблема:** в live-диалоге про инвестиции клиент написал `публицист`, но `scenario_iteration_operator_handoff` сработал раньше LLM-planner и вывел `ЖК «выбранные варианты»`. Это было неверно архитектурно: код не должен regex-ом угадывать ЖК, но и не должен обходить LLM-orchestrator.
- **Фикс:** `DIALOG_STATE_PLANNER_PROMPT` теперь явно требует распознавать выбор варианта по номеру, частичному названию, разговорному сокращению, отсутствию `ЖК` или небольшой опечатке, если по `visible_options` / `last_options` однозначно понятно, какой объект выбран. В `selected_option_name` planner возвращает каноническое точное `name` из памяти.
- **Routing:** `scenario_iteration_operator_handoff` перенесён после `plan_dialog_state` и `_apply_dialog_plan_to_state`, чтобы LLM сначала выставил `selected_option`, а handoff уже использовал выбранный ЖК. Код по-прежнему не принимает новые ЖК на веру: он только валидирует `selected_option_name` против памяти.
- **Safety:** если выбранный ЖК всё-таки неизвестен, текст говорит `выбранные варианты` без фальшивого префикса `ЖК`.
- **Verification:** локально `py_compile`, `H029 45/45`, `UX_E2E 26/26`, `stateful 1/1`, `compare 1/1`; VPS backup `backups/deploy-20260709-171121`; VPS `H029 45/45`, `UX_E2E 26/26`; `novostroy-bot.service` active с PID `692241`. Дополнительно после deploy: VPS `--suite dialog` прошёл `3/3`; handler-like smoke с `ЖК «Публицист»` в памяти подтвердил цепочку `followup_classifier → selected_option_action="set" → selected_option_set → scenario_iteration_operator_handoff`, без placeholder `выбранные варианты`.

---

## 2026-07-09 — Scenario iteration handoff на 3-й итерации 🔁

### Бот раньше ведёт к оператору, когда сценарий уже упёрся в live-условия
- **Проблема:** в живом диалоге `двушка для семьи → все без ПВ? → ЖК Дюна → семейная` бот слишком долго продолжал отвечать через search/fact-check и только поздно подводил к оператору, хотя точные условия без ПВ/ипотеки зависят от актуальных банковских и объектных программ.
- **Фикс:** `scripts/chat_tester_bot.py` теперь хранит `active_scenario` и считает итерации внутри одного сценария для всех сценариев, а не только для `без ПВ`.
- **Behavior:** на третьей итерации одного сценария, если уже есть выбранный ЖК/варианты/активная задача, бот делает мягкий `operator_handoff`: объясняет, почему дальше лучше не гадать, и спрашивает, хочет ли клиент оставить номер. `awaiting_phone` включается только после согласия.
- **Verification:** локально `H029 45/45`, `UX_E2E 25/25`, `stateful 1/1`, `compare 1/1`; VPS backup `backups/deploy-20260709-140309`; VPS `H029 45/45`, `UX_E2E 25/25`; `novostroy-bot.service` active с PID `660732`; markers `active_scenario`, `_scenario_iteration_handoff_needed`, `scenario_iteration_operator_handoff`, `scenario_iteration_limit_operator_handoff` присутствуют.

---

## 2026-07-09 — Parallel fallback race для main_search 🏁

### Если Google fallback тоже падает, бот пробует другого provider параллельно
- **Проблема:** после broad-search фикса live-запрос `двушка для семьи` всё равно мог уйти в safe fallback: основной `google/gemini-3.1-flash-lite-preview` падал через OpenRouter/Google `Corrupted thought signature`, затем `google/gemini-3.5-flash` как fallback тоже оставался Google-provider и мог упасть тем же классом ошибки.
- **Фикс:** `scripts/chat_tester_bot.py` теперь после upstream/safe error, пустого search-result или underfilled broad shortlist запускает fallback race: `google/gemini-3.5-flash` и `deepseek/deepseek-v4-flash` одновременно.
- **Selection:** побеждает первый пригодный результат, а не просто самый быстрый. Пригодный = не safe fallback, не пустой `facts=[]`/`near=[]`, и для broad search не `underfilled`.
- **Safety:** быстрый непригодный ответ игнорируется, бот ждёт второй provider. Если оба fallback-ответа непригодны, клиент получает безопасную ошибку; бот не выдумывает ЖК.
- **Verification:** локально `py_compile`, `H029 45/45`, `UX_E2E 24/24`, `stateful 1/1`, `compare 1/1`; deterministic race test подтвердил, что быстрый invalid Gemini 3.5 не побеждает, а валидный `deepseek/deepseek-v4-flash` выбирается; VPS backup `backups/deploy-20260709-133342`; VPS `H029 45/45`, `UX_E2E 24/24`; `novostroy-bot.service` active с PID `654932`; markers `MAIN_SEARCH_FALLBACK_MODELS`, `_is_usable_search_result`, `_search_fallback_race`, `deepseek/deepseek-v4-flash` присутствуют.

---

## 2026-07-09 — Broad search count=3 и underfilled fallback 🔎

### Широкий подбор больше не должен схлопываться в один ЖК
- **Проблема:** live-запрос `двушка для семьи` иногда возвращал один ЖК, хотя это широкий подбор и клиент ожидает несколько вариантов. В prod-логе `search_response` содержал только `Жилой район «Скандинавия»`, а formatter ничего не обрезал — короткий список приходил уже из search-layer.
- **Фикс:** `scripts/chat_tester_bot.py` теперь кодово ставит `count >= 3` для broad search (`search`, `repeat_search`, `family`, `investment`, `rental` и широкие ипотечные/платёжные сценарии). `fact_check`, `operator` и выбранный конкретный ЖК не получают принудительный `count`.
- **Fallback:** если broad search вернул только один объект (`0 < facts + near < 2`), это считается `underfilled shortlist`; бот делает один fallback-запрос через `google/gemini-3.5-flash`.
- **Safety:** если fallback всё равно дал мало, бот отвечает честно по доступному варианту и логирует `_underfilled_shortlist`, но не придумывает дополнительные ЖК.
- **Verification:** локально `h029 45/45`, `UX_E2E 24/24`, `stateful 1/1`, `compare 1/1`; VPS backup `backups/deploy-20260709-123955`; VPS `h029 45/45`, `UX_E2E 24/24`; `novostroy-bot.service` active с PID `645436`; prod smoke `двушка для семьи` вернул 3 ЖК и metadata `_first_attempt_underfilled_shortlist=True`, `_search_fallback_model=google/gemini-3.5-flash`.

---

## 2026-07-09 — Gemini 3.5 Flash fallback для main_search 🔁

### Search-слой больше не падает сразу после пустого/ошибочного Gemini 3.1 ответа
- **Проблема:** основной `main_search` на `google/gemini-3.1-flash-lite-preview` иногда возвращал upstream/OpenRouter ошибку или валидный, но пустой search-result. В таком случае бот мог сразу уходить в безопасную ошибку, хотя повтор через другую быструю Gemini-модель часто восстанавливает поиск.
- **Фикс:** `scripts/chat_tester_bot.py` теперь держит основной поиск на `google/gemini-3.1-flash-lite-preview`, но при upstream/safe error или пустом `facts=[]` + `near=[]` делает один fallback-запрос через `google/gemini-3.5-flash`.
- **Safety:** если fallback тоже пустой или ошибочный, бот возвращает безопасную ошибку и не придумывает квартиры без MCP/search фактов.
- **Verification:** локально проверены deterministic-сценарии fallback; на VPS `/home/neiro/novostroy-bot/scripts/chat_tester_bot.py` содержит `MAIN_SEARCH_FALLBACK_MODEL="google/gemini-3.5-flash"`, `MAIN_SEARCH_FALLBACK_ENABLED=1`, `_is_empty_search_result`; `python3 -m py_compile scripts/chat_tester_bot.py` ✅; `novostroy-bot.service` active.
