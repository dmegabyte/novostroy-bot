...
def test_runtime_summary_sanitizer_drops_raw_nested_unknowns(tmp_path: Path, monkeypatch) -> None:
    ...
    assert runtime["call_counts"] == {"planner": 1, "search": 1, "selected_enrichment": 0, "selected_capability": 0, "gateway_attempts": 5, "scenario_search": 2, "answer": 3}
...
def test_response_composer_attempt_diagnostic_sanitizer_is_allowlisted(tmp_path: Path, monkeypatch) -> None:
    ...
    event = mod.append_event(
        ...
        response_composer={
            ...
            "semantic_diagnostics": [
                {"stage": "writer", "categories": ["numeric_price_not_in_canonical", "numeric_transit_not_in_canonical", "sensitive_claim", "secret"], "matched_text": "99 млн", "position": 8},
    ...
    assert event["response_composer"]["semantic_diagnostics"] == [
        {"stage": "writer", "categories": ["numeric_price_not_in_canonical", "numeric_transit_not_in_canonical", "sensitive_claim"]}
    ]
