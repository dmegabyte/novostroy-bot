...
def _safe_capability_outcome(value: Any) -> dict[str, Any] | None:
    ...
    out: dict[str, Any] = {"requested_facts": list(dict.fromkeys(str(item) for item in facts if str(item) in _SAFE_CAPABILITY_FACTS))[:8], "status": status, "request_count": _bounded_int(value.get("request_count"), 0, 1), "evidence_status": evidence, "accepted_count": _bounded_int(value.get("accepted_count"), 0, 10), "rejected_count": _bounded_int(value.get("rejected_count"), 0, 10), "identity_match": value.get("identity_match") if isinstance(value.get("identity_match"), bool) else None, "active_root": value.get("active_root") if isinstance(value.get("active_root"), bool) else None, "root_state": value.get("root_state") if value.get("root_state") in _SAFE_CAPABILITY_ROOT_STATES else "unknown"}
