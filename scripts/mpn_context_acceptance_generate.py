#!/usr/bin/env python3
"""Build label-blind MPN navigation/context-gate predictions."""
...
DEFAULT_INPUTS = PROJECT_ROOT / "tests" / "fixtures" / "mpn_context_acceptance_inputs_v1.json"
DEFAULT_OUTPUT = Path("/tmp/opencode/mpn_context_acceptance_predictions_v1.json")
MAX_SOURCES = 2
MAX_LINES = 80
MAX_CHARS = 8000
...
def _abstain(case: dict[str, str], *, route: str, reason: str) -> dict[str, Any]:
    return {
        ...
        "transitive_or_cross_root_claim": False,
    }
...
def predict_case(case: dict[str, str]) -> dict[str, Any]:
    ...
    try:
        nav = navigation.navigate(code, project_id="mpn")
    ...
    try:
        ctx = gate.run_gate(
            ...
            definition_of_done="exact MPN diagnostic owner symbol under strict local STOP-2 budget",
    ...
    return {
        ...
        "owner_path": str(spec.get("owner_path") or spec.get("target_owner") or ""),
        "symbol": str(spec.get("target") or ""),
        ...
        "cross_project_notebooks": int(trace.get("cross_project_notebooks") or 0),
...
def generate(input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": 1,
        "project_id": "mpn",
        "harness": "mpn_context_acceptance_v1",
