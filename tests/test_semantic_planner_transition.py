...
from nmbot_v2.contracts import IntentPlanV3, OptionCard, SafeTurnContext, SemanticPlan, Stage, TurnAction
...
class _NoProviderSession:
    ...
    def post(self, *args: Any, **kwargs: Any) -> Any:
        self.calls += 1
        raise AssertionError("secondary-market boundary must not call the planner provider")


class _NoSearchService:
    def __init__(self) -> None:
        self.calls = 0

    def search(self, *args: Any, **kwargs: Any) -> Any:
        self.calls += 1
        raise AssertionError("secondary-market boundary must not invoke search")
...
@pytest.mark.parametrize("user_text", ["Москва, вторичную однушку", "ищу квартиру на вторичном рынке"])
def test_secondary_housing_is_deterministic_catalog_boundary_without_provider_or_search(user_text: str) -> None:
    intent_plan = asyncio.run(planner.plan_intent_v3(_NoProviderSession(), user_text=user_text, state={}))

    assert intent_plan["goal"] == "off_topic"
    typed_plan = IntentPlanV3.from_dict({key: value for key, value in intent_plan.items() if key in IntentPlanV3.__dataclass_fields__})
    transition = derive_transition_v3(typed_plan, ConversationState())
    assert transition.stage is Stage.OFF_TOPIC
    assert transition.action is TurnAction.ANSWER_OFF_TOPIC

    search = _NoSearchService()
    turn = TurnProcessor(
        planner=_StaticPlanner(SemanticPlan(operation="freeform", domain_relation="off_topic")),
        search_service=search,
    ).process(SafeTurnContext("secondary-boundary", user_text), ConversationState())

    assert turn.stage is Stage.OFF_TOPIC
    assert search.calls == 0
    assert turn.response_text.endswith("Вернёмся к подбору квартиры?")
    assert turn.response_text.count("?") == 1


@pytest.mark.parametrize("user_text", ["двушка в Москве", "второй вариант"])
def test_ordinary_new_build_request_and_ordinal_are_not_secondary_housing(user_text: str) -> None:
    assert planner.secondary_housing_catalog_boundary(user_text) is None


def test_project_ready_secondary_constraint_remains_a_new_build_search_field() -> None:
    result = planner._with_canonical_fields(
        {},
        semantic_search(constraints_delta={"hard": {"project_ready_secondary": True}, "preferences": {}, "unknown": {}}),
        state={},
    )

    assert result["action"] == "search"
    assert result["search_policy"] == "required"
    assert result["constraints_patch"]["hard"] == {"project_ready_secondary": True}


@pytest.mark.parametrize("user_text", ["есть ипотека?", "ипотечные условия есть?"])
def test_selected_financing_is_deterministic_v3_boundary_without_provider(user_text: str) -> None:
    session = _NoProviderSession()
    state = {"selected_option": {"name": "ЖК Лучи"}}

    intent_plan = asyncio.run(
        planner.plan_intent_v3(
            session,
            user_text=user_text,
            state=state,
            selected_object={"canonical_name": "ЖК Лучи"},
        )
    )

    assert session.calls == 0
    assert intent_plan["goal"] == "answer_selected"
    assert intent_plan["viewpoint"] == "financing"
    assert intent_plan["selected_option_name"] == "ЖК Лучи"
    assert intent_plan["requested_facts"] == ["mortgage_terms"]
    executable = compile_executable_turn_v3(
        intent_plan,
        ConversationState(visible_options=(OptionCard(name="ЖК Лучи"),)),
        query_text=user_text,
    )
    assert executable.stage is Stage.SELECTED_OBJECT
    assert executable.action is TurnAction.ANSWER_SELECTED_OPTION
    assert executable.scope == "one"


def test_selected_financing_boundary_skips_all_current_and_pending_consent() -> None:
    selected = {"canonical_name": "ЖК Лучи"}
    state = {"selected_option": {"name": "ЖК Лучи"}}

    assert planner._selected_financing_intent_plan_v3(
        "по всем есть ипотека?", state=state, selected_object=selected, pending_scenario={}
    ) is None
    assert planner._selected_financing_intent_plan_v3(
        "ипотека?", state=state, selected_object=selected, pending_scenario={"id": "financing_consent"}
    ) is None
    assert planner._selected_financing_intent_plan_v3(
        "есть ипотека?", state={}, selected_object={}, pending_scenario={}
    ) is None


def test_selected_financing_v3_boundary_never_renders_raw_mortgage_terms_before_consent() -> None:
    card = OptionCard(name="ЖК Лучи", mortgage_terms="Ставка 6,2%, первоначальный взнос 3 млн, одобрение за день")
    initial_state = ConversationState(visible_options=(card,), selected_option_name=card.name)
    intent_plan = asyncio.run(
        planner.plan_intent_v3(
            _NoProviderSession(),
            user_text="есть ипотека?",
            state={"selected_option": {"name": card.name}},
            selected_object={"canonical_name": card.name},
        )
    )
    executable = compile_executable_turn_v3(intent_plan, initial_state, query_text="есть ипотека?")

    turn = TurnProcessor(_StaticPlanner(executable)).process(
        SafeTurnContext("selected-financing-boundary", "есть ипотека?"), initial_state
    )

    assert turn.response_plan.recipe_id == "selected_financing"
    assert turn.response_plan.operator_prompt is False
    assert turn.response_text.rstrip().endswith("Проверить условия по этому ЖК?")
    for prohibited in ("6,2%", "3 млн", "ставк", "первоначальн", "одобрени"):
        assert prohibited not in turn.response_text.lower()
