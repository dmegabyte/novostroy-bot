...
### Правило минимальной архитектуры

Сначала всегда ищем самое простое решение в существующем owner-контракте.
Нельзя добавлять отдельный classifier, router, prompt или adapter только для
того, чтобы повторить уже существующую semantic/search логику. Для обычного
поиска один unified search prompt является владельцем `action`, параметров и
MCP-поиска; код проверяет результат, применяет state и при необходимости
передаёт canonical material presenter-у. V6 обозначает версию runtime и
bounded enrichment, но не отдельный модельный слой.

Новый слой допускается только после source-backed доказательства, что текущий
owner не может выполнить нужный контракт. Исключение должно быть зафиксировано
в `PROMPT_ARCHITECTURE.md` и иметь план удаления дублирующего слоя.
