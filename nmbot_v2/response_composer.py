...
def _unknown_numbers_against_sources(text: str, writer_text: str, brief: ResponseBrief) -> bool:
    allowed = set(_allowed_number_tokens(brief))
    for source in (writer_text,):
        for token in re.findall(r"\d+(?:[\s.,–-]*\d+)*", source):
            digits = re.sub(r"\D", "", token)
            if digits:
                allowed.add(digits)
    for token in re.findall(r"\d+(?:[\s.,–-]*\d+)*", text):
        digits = re.sub(r"\D", "", token)
        if len(digits) >= 2 and digits not in allowed:
...
def _semantic_validation_diagnostics(
    *,
    ...
    for error in errors:
        ...
        category = {
            "unknown_number_or_sensitive_claim": "numeric_not_in_canonical",
            "unsupported_sensitive_claim": "sensitive_claim",
        }.get(code)
        if category and category not in categories:
            categories.append(category)
    return [{"stage": stage, "categories": categories[:2]}] if categories else []
...
def _fallback_result(fallback_text: str, first: ComposerAttemptResult, *, attempts: int | None = None) -> ComposerAttemptResult:
    return ComposerAttemptResult(status="fallback", text=fallback_text, errors=first.errors, error_category=first.error_category, error_code=first.error_code, attempts=attempts or first.attempts, attempt_summaries=first.attempt_summaries)
...
def _unknown_numbers(text: str, brief: ResponseBrief) -> bool:
    allowed = _allowed_number_tokens(brief)
    for token in re.findall(r"\d+(?:[\s.,–-]*\d+)*", text):
        digits = re.sub(r"\D", "", token)
        if len(digits) >= 2 and digits not in allowed:
...
def _allowed_number_tokens(brief: ResponseBrief) -> set[str]:
    out = {"1", "2", "3"}
