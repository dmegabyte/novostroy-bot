...
def test_allowlist_includes_source_backed_h108_keys_only():
    assert {
        "name", "ads", "price_min", "location_name", "is_near", "why_close", "differences", "object_id",
        "ads_add.stat_price", "location_2.ecology_rating", "payment_by_installments",
        "house.finishing_list", "garage_count",
    } <= FACT_FIELDS
    assert "guessed" not in FACT_FIELDS
    tree = ast.parse((ROOT / "nmbot_v6" / "simple_contract.py").read_text(encoding="utf-8"))
    imported_roots = {
        name.name.split(".", 1)[0]
    ...
    runtime_roots = {name for name in imported_roots if re.fullmatch(r"nmbot_v\d+", name)}
